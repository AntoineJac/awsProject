...
(function (root, factory) {
  ...
  Events = Postmonger.Events = function () {
    ...
    self.on = function (events, callback, context) {
      let calls; let event; let node; let tail; let
        list;
    ...
    self.off = function (events, callback, context) {
      ...
      let event; let node; let tail; let cb; let
        ctx;
    ...
    self.trigger = function (events) {
      let event; let node; let calls; let tail; let args; let all; let
        rest;
  ...
  Session = Postmonger.Session = function () {
    const args = (arguments.length > 0) ? Array.prototype.slice.call(arguments, 0) : [{}];
    const connections = [];
    const incoming = new Events();
    const outgoing = new Events();
    const self = this;
    let connection; let i; let j; let l; let ln; let
      postMessageListener;
